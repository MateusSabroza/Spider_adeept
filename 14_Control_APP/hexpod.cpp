...
ultrasonic();
if(a<=20 && a>0){
  for(int i=0;i<led_numbers;i++) strip.setPixelColor(i,strip.Color(255,0,0));
  strip.show();

  // Scan left
  s12.write(angle12 + 45);
  delay(400);
  ultrasonic();
  long leftDist = (a == 0) ? 999 : a;

  // Scan right
  s12.write(angle12 - 45);
  delay(400);
  ultrasonic();
  long rightDist = (a == 0) ? 999 : a;

  // Return head to center
  s12.write(angle12);
  delay(200);

  // Go to the clearer side
  if(leftDist >= rightDist){
    for(int i=0;i<4;i++){ turnleft(); if(Serial.available()>0) break; }
  } else {
    for(int i=0;i<4;i++){ turnright(); if(Serial.available()>0) break; }
  }
} else {
  for(int i=0;i<led_numbers;i++) strip.setPixelColor(i,strip.Color(0,200,0));
  strip.show();
}
